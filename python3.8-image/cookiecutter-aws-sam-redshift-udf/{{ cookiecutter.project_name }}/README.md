...
```bash
{{ cookiecutter.project_name }}$ sam logs -n {{ cookiecutter.function_name }} --stack-name <Name-of-your-deployed-stack> --tail
```
...
* **`make invoke`**: `sam local invoke {{ cookiecutter.function_name }} --event events/redshift_udf_event.json`
...
```bash
{{ cookiecutter.project_name }}$ pipenv lock -r > app/requirements.txt
```
